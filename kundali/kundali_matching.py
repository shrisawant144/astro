...
_VARNA = {
    "Cancer": 0,
    "Scorpio": 0,
    "Pisces": 0,  # Brahmin (water)
    "Leo": 1,
    "Sagittarius": 1,
    "Aries": 1,  # Kshatriya (fire)
    "Gemini": 2,
    "Libra": 2,
    "Aquarius": 2,  # Vaishya (air)
    "Taurus": 3,
    "Virgo": 3,
    "Capricorn": 3,  # Shudra (earth)
}
...
def _varna(sign_m, sign_f):
    ...
    return (
        score,
        1,
        f"Male Varna: {['Brahmin','Kshatriya','Vaishya','Shudra'][vm]}, Female: {['Brahmin','Kshatriya','Vaishya','Shudra'][vf]}",
    )
...
def _bhakoota(sign_m, sign_f):
    ...
    if pair in bad_pairs:
        return (
            0,
            7,
            f"Inauspicious Bhakoota ({diff_mf}/{diff_fm}) — relationship friction",
        )
...
def _nadi(nak_m, nak_f):
    ...
    if nm == nf:
        return (
            0,
            8,
            f"Same Nadi ({nadi_names[nm]}) — Nadi Dosha: health/progeny concerns",
        )
...
def _stree_deergha(nak_m, nak_f):
    ...
    if diff >= 9:
        return (
            True,
            f"Stree Deergha: male star {diff} nakshatras ahead — husband lives long",
        )
    return (
        False,
        f"Stree Deergha not satisfied (diff={diff}): longevity concern for husband",
    )
...
def match_kundalis(result_male, result_female):
    ...
    doshas["Nadi Dosha"] = {
        "present": nadi_dosha,
        "detail": "Same Nadi — Nadi Dosha present"
        if nadi_dosha
        else "Different Nadi — no Nadi Dosha",
    }
