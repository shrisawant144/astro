...
INAUSPICIOUS_TITHIS = {
    ...
    12,
    14,
}  # Chaturthi, Ashtami, Navami, Dwadashi, Chaturdashi
...
def evaluate_muhurtha(jd, birth_result, lat=0.0, lon=0.0, purpose="general"):
    ...
    panchaka_data = calculate_panchaka(
        panchanga["tithi"],
        panchanga["vara"],
        panchanga["nakshatra_idx"],
        lagna_sign_idx,
        current_moon_sign_idx,
    )
    if panchaka_data["has_panchaka"]:
        score = max(0, score - 15)
        warnings.append(
            f"Panchaka Dosha: {panchaka_data['type']} — {panchaka_data['description']}"
        )

    # --- Purpose-specific adjustments ---
    purpose_adjustments = {
        "marriage": {
            "good_vara": {"Wednesday", "Thursday", "Friday"},
            "bad_vara": {"Tuesday", "Saturday", "Sunday"},
        },
        "business": {
            "good_vara": {"Wednesday", "Thursday"},
        ...
        "medical": {
            "good_vara": {"Monday", "Wednesday", "Thursday"},
            "bad_vara": {"Tuesday", "Saturday"},
        },
        "education": {"good_vara": {"Wednesday", "Thursday"}, "bad_vara": set()},
        "property": {
            "good_vara": {"Wednesday", "Thursday", "Friday"},
            "bad_vara": {"Tuesday", "Saturday"},
        },
...
def find_best_muhurtha(
    birth_result, start_jd, days=7, purpose="general", lat=0.0, lon=0.0, min_score=65
):
    """
