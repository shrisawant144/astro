...
def get_birth_bird(moon_nakshatra: str) -> str:
    ...
    try:
        ...
        for i, n in enumerate(NAKSHATRA_NAMES):
            if (
                moon_nakshatra.lower() in n.lower()
                or n.lower() in moon_nakshatra.lower()
            ):
                nak_idx = i
                break
        else:
...
def get_yama_info(
    query_jd: float, sunrise_jd: float, sunset_jd: float, weekday: int
) -> dict:
    """
    ...
    if is_day:
        ...
        if query_jd < sunrise_jd:
            ...
            prev_sunset_jd = (
                sunset_jd - 1.0
            )  # approximate; caller should pass correct data
...
def get_full_day_forecast(
    birth_bird: str, weekday: int, sunrise_jd: float, sunset_jd: float
) -> list:
    """
...
def calculate_pancha_pakshi(
    result: dict, query_dt: datetime.datetime | None = None
) -> dict:
    """
    ...
    all_bird_activities = {
        b: {
            ...
            "strength": ACTIVITY_STRENGTH.get(
                yama_info["yama_activities"].get(b, "Sleeping"), 3
            ),
        }
        for b in BIRDS
    }
    ...
    return {
        ...
        "nak_index": NAKSHATRA_NAMES.index(moon_nak)
        if moon_nak in NAKSHATRA_NAMES
        else -1,
